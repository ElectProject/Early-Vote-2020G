...
FL_out_party <- party_shell_returned

FL_out_party[1,2] <- state_stats[10,10] - state_stats[10,29] 
FL_out_party[2,2] <- state_stats[10,11] - state_stats[10,30]
FL_out_party[3,2] <- state_stats[10,12] - state_stats[10,31]
FL_out_party[4,2] <- state_stats[10,13] - state_stats[10,32]
FL_out_party[5,2] <- state_stats[10,5] - state_stats[10,6]

FL_out_party[1,4] <- state_stats[10,29] 
FL_out_party[2,4] <- state_stats[10,30] 
FL_out_party[3,4] <- state_stats[10,31] 
FL_out_party[4,4] <- state_stats[10,32] 
FL_out_party[5,4] <- state_stats[10,6] 

FL_out_party[1,5] <- state_stats[10,10] 
FL_out_party[2,5] <- state_stats[10,11] 
FL_out_party[3,5] <- state_stats[10,12] 
FL_out_party[4,5] <- state_stats[10,13] 
FL_out_party[5,5] <- state_stats[10,5] 

FL_out_party$Frequency <- 100*FL_out_party$Count/FL_out_party[5,2]

colnames(FL_out_party) <- c("Party", "Outstanding Ballots", "Freq. Distribution", "Returned Ballots", "Requested Ballots")
...
Florida did not update their county data files today. Statewide data are current, but county maps and statistics are one day behind. I will post new data if this issue is resolved.
...
Total Voted as Percentage of Registered Voters: **`r paste(round(100*as.integer(state_stats[10,8])/as.integer(sum(FL_stats$Reg.Voters)), digits = 1),"%", sep = "")`**
...
hcmap(map = "countries/us/us-fl-all", data = FL_2020g_map_data,
  ...
  hc_title(text ="Total Voted as Percent of Registered Voters") %>%
...
### Mail Balllots Outstanding

``` {r echo = FALSE}
kable(FL_out_party,format.args = list(big.mark = ",", 
  scientific = FALSE), digits = 1) %>%
  kable_styling(bootstrap_options = "striped", full_width = F, position = "left")
```
