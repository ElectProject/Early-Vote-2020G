...
Montana voters have cast **`r format(as.numeric(state_stats[27,6]), big.mark =",")`** mail ballots.
...
``` {r echo = FALSE}
datatable(MT_stats_returns, colnames = c("County", "Mail Ballots Returned", "Mail Ballot Requests", "Percent Returned"), rownames = F) %>%
...
Montana voters have requested **`r format(as.numeric(state_stats[27,5]), big.mark =",")`** mail ballots.

Montana gave its local election officials the option of conducting an all-mail ballot election. All but 10 smaller counties have chosen to run an all-mail ballot election. For now, ballot requests reported here are counts of the state's registered voters.
